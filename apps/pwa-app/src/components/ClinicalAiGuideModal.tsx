...
export const ClinicalAiGuideModal: React.FC<ClinicalAiGuideModalProps> = ({
  ...
  const runClinicalRagInference = async () => {
    ...
    const queryText = `Patient ${examData.age || 30}yo ${examData.sex || 'Unknown'}. Vitals: Temp ${examData.vitals?.temperature || 'N/A'}C, BP ${examData.vitals?.bloodPressure || 'N/A'}, HR ${examData.vitals?.heartRate || 'N/A'}. Chief Complaint: ${examData.chiefComplaint || 'N/A'}. Symptoms: ${(examData.symptoms || []).join(', ')}. Notes: ${examData.notes || 'N/A'}`;
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Plain English steps + Formal clinical terms for volunteer healthcare workers</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400">Preparing easy step-by-step guidance for your patient</p>
              </div>
            </div>
          )}

          {errorText && !loading && (
            <div className="p-4 rounded-xl border border-rose-500/30 bg-rose-950/40 text-rose-200 space-y-3">
              <div className="flex items-center gap-2 font-bold text-sm text-rose-300">
                <AlertTriangle className="w-5 h-5 text-rose-400" />
                Clinical Assistant Notice
              </div>
              <p className="text-xs text-slate-300">{errorText}</p>
              <button
                onClick={runClinicalRagInference}
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 text-rose-200 text-xs font-semibold border border-rose-500/40 transition-colors"
              >
              ...
              >
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  {parsedGuide.referral_reasoning && (
                    <p className="text-xs text-slate-200 leading-relaxed">{parsedGuide.referral_reasoning}</p>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-2.5">
                  {parsedGuide.differential_diagnoses.map((diag, i) => (
                    <div key={i} className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-1.5">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-sm text-teal-300">{diag.friendly_title}</span>
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed pt-0.5">{diag.plain_explanation}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2.5">
                  {parsedGuide.recommended_treatments.map((t, i) => (
                    <div key={i} className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-1.5">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-sm text-emerald-300">{t.medication_name}</span>
                        <span className="text-xs text-slate-400 bg-slate-900 px-2 py-0.5 rounded border border-slate-800">
                          {t.dosage_details}
                        </span>
                      </div>
                      <p className="text-xs font-medium text-slate-200">{t.plain_instructions}</p>
                      {t.friendly_notes && (
